...
// ServiceRaftDBDisk provides service transport over raft
type ServiceRaftDBDisk struct {
...
func (s *ServiceRaftDBDisk) Init() error {
	return nil
}

// Metrics put a request on the raft (only leaders should do this)
func (s *ServiceRaftDBDisk) Metrics(ctx context.Context, req []byte) ([]byte, error) {
...
func (s *ServiceRaftDBDisk) Play() (err error) {
	if s.Playing {
		return nil
	}
	s.Playing = true
	...
	nodeConfig := config.Config{
		...
		ElectionRTT:            s.RaftRTTsPerElection,
		HeartbeatRTT:           s.RaftRTTsPerHeartbeat,
	...
	stateMachineFactory := func(clusterID uint64, nodeID uint64) statemachine.IOnDiskStateMachine {
	...
	bootstrapOrJoin := func() error {
		...
		if initCluster {
			...
			if err := nodeHost.StartOnDiskCluster(initialMembers, false, stateMachineFactory, nodeConfig); err != nil {
			...
			initialMembers := make(map[uint64]string)
			if err := nodeHost.StartOnDiskCluster(initialMembers, true, stateMachineFactory, nodeConfig); err != nil {
				core.Log.Fatal(fmt.Errorf("failed to add cluster: %w", err).Error())
			}
...
func (s *ServiceRaftDBDisk) JoinRequest(joinHostport, joinPeerHostport string, nodeID uint64, clusterID uint64, autoJoin bool) error {
...
func (s *ServiceRaftDBDisk) LeaderJoinHandler(ctx context.Context, req []byte) (res []byte, err error) {
...
func (s *ServiceRaftDBDisk) WriteHandler(ctx context.Context, req []byte) ([]byte, error) {
	start := time.Now()
	...
	// core.Log.Warnf("ServiceRaftDB: WriteHandler: timeout is %d", duration)
	// }
...
func (s *ServiceRaftDBDisk) ReadHandler(ctx context.Context, req []byte) ([]byte, error) {
	start := time.Now()
	...
	if err != nil {
		return nil, fmt.Errorf("ServiceRaftDB: ReadHandler: SyncRead: %w", err)
	}
...
// Open opens the existing on disk state machine.
func (s *ServiceRaftDBDisk) Open(stopc <-chan struct{}) (uint64, error) {
	return s.RaftableService.Open(stopc)
}

// Close closes the IOnDiskStateMachine
func (s *ServiceRaftDBDisk) Close() error {
	return s.RaftableService.Stop()
}

// Sync synchronizes the RAFT fsm
func (s *ServiceRaftDBDisk) Sync() error {
	return s.RaftableService.Sync()
}

// Update applys a change to the FSM
func (s *ServiceRaftDBDisk) Update(entries []statemachine.Entry) ([]statemachine.Entry, error) {
	for i, entry := range entries {
		// Record request time and size
		start := time.Now()
		defer servicePutTimeMetrc.Observe(float64(time.Since(start)))
		servicePutReqSizeMetrc.Observe(float64(len(entry.Cmd)))
		...
		// Do the write
		handlerRes, handlerErr, writeErr := s.RaftableService.Write(ctx, entry.Index, entry.Cmd, s.RaftableService.Handler)

		// Record the response size.
		servicePutResSizeMetrc.Observe(float64(len(handlerRes)))

		// Bomb if there is an error writing the proposalIdx to the db
		if writeErr != nil {
			return entries, writeErr
		}

		// Pass handler errors through as entry.Result.Data
		if handlerErr != nil {
			entries[i].Result = statemachine.Result{Value: smResultError, Data: []byte(handlerErr.Error())}
		} else {
			entries[i].Result = statemachine.Result{Value: smResultSuccess, Data: handlerRes}
		}
	}
...
func (s *ServiceRaftDBDisk) Lookup(input interface{}) (interface{}, error) {
	start := time.Now()
...
func (s *ServiceRaftDBDisk) RecoverFromSnapshot(snapshotStream io.Reader, abort <-chan struct{}) error {
...
// PrepareSnapshot gets a snapshot ready.
func (s *ServiceRaftDBDisk) PrepareSnapshot() (interface{}, error) {
	snapshot, err := s.RaftableService.SnapshotMake()
	if err != nil {
		return nil, err
	}
...
func (s *ServiceRaftDBDisk) SaveSnapshot(snapshot interface{}, snapshotStream io.Writer, abort <-chan struct{}) error {
	serviceRaftSnapshot := snapshot.(ServiceRaftSnapshot)
	return serviceRaftSnapshot.Write(snapshotStream)
}
